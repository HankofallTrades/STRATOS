...
- `src/components/layout/ProtectedAppEntry.tsx`
  ...
  - **Guardrail: this gate resolves once per entry, never per navigation.** The effect is keyed on the gate state via `shouldResolveProtectedSession`, deliberately not on `location.pathname`. Re-resolving returns the gate to `checking`, which unmounts `ProtectedAppShell` and every piece of shell-level state below it — the Coach conversation, and the refs `useProactiveEngine` compares against to spot a transition, so finishing a workout stopped looking like a change and the `workout_finished` insight never fired (I-23). Keeping a session valid over time is `AuthProvider`'s job: it subscribes to `onAuthStateChange`, and `ProtectedRoute` redirects when the session goes away.

...
import { 
  calculateOneRepMax as utilCalculateOneRepMax,
  getWeightSuggestions as utilGetWeightSuggestions,
  getLastPerformance as utilGetLastPerformance,
  getLastSetPerformance as utilGetLastSetPerformance 
} from "@/lib/workout/workoutUtils";
import { 
  EXERCISES_STORAGE_KEY, 
  WORKOUT_HISTORY_STORAGE_KEY, 
  LAST_USED_EQUIPMENT_STORAGE_KEY, 
...
interface WorkoutContextType {
  ...
  updateExerciseEquipment: (workoutExerciseId: string, equipmentType: EquipmentType) => void;
...
interface LastUsedEquipment {
  [exerciseId: string]: EquipmentType;
}
...
export const WorkoutProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Initialize from local storage (uses imported constants)
  useEffect(() => {
  ...
  // Save to local storage (uses imported constants)
  useEffect(() => {
    if (exercises.length) {
  ...
  // Load/Save last used equipment (uses imported constant)
  useEffect(() => {
    const storedLastUsedEquipment = localStorage.getItem(LAST_USED_EQUIPMENT_STORAGE_KEY);
  ...
  useEffect(() => {
    localStorage.setItem(LAST_USED_EQUIPMENT_STORAGE_KEY, JSON.stringify(lastUsedEquipment));
  }, [lastUsedEquipment]);
  
  // Load/Save custom variations (uses imported constant)
  useEffect(() => {
    const storedVariations = localStorage.getItem(EXERCISE_VARIATIONS_STORAGE_KEY);
    if (storedVariations) {
  ...
  useEffect(() => {
    localStorage.setItem(EXERCISE_VARIATIONS_STORAGE_KEY, JSON.stringify(exerciseVariations));
  }, [exerciseVariations]);
  ...
  const updateOneRepMax = (exerciseId: string, weight: number, reps: number) => {
    const oneRepMax = utilCalculateOneRepMax(weight, reps);
    setExercises((prev) =>
      prev.map((ex) =>
        ex.id === exerciseId
  ...
  const updateLastUsedEquipment = (exerciseId: string, equipmentType: EquipmentType) => {
  ...
  const getEquipmentTypeForExercise = (exerciseId: string): EquipmentType => {
  ...
  const updateExerciseEquipment = (workoutExerciseId: string, equipmentType: EquipmentType) => {
    if (!currentWorkout) return;
  ...
  // Define context value methods that call the aliased imported utility functions
  const provideGetWeightSuggestions = (exerciseId: string): WeightSuggestion[] => {
    return utilGetWeightSuggestions(exerciseId, exercises);
  };

  const provideGetLastPerformance = (exerciseId: string): { weight: number; reps: number } | null => {
    return utilGetLastPerformance(exerciseId, workoutHistory);
  };

  const provideGetLastSetPerformance = (
    exerciseId: string,
    setNumber: number, 
    equipmentType?: EquipmentType,
    variation?: string
  ): { weight: number; reps: number } | null => {
    return utilGetLastSetPerformance(exerciseId, setNumber, workoutHistory, equipmentType, variation);
  };
  ...
  const value = {
    ...
    workoutHistory,
    workoutTime,
    getWeightSuggestions: provideGetWeightSuggestions,
    getLastPerformance: provideGetLastPerformance,
    getLastSetPerformance: provideGetLastSetPerformance,

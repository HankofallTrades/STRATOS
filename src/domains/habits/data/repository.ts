...
export const getHabitCompletionDates = async (
  userId: string,
  habitId: string,
  limit: number = 365
): Promise<string[]> => {
  if (!userId) throw new Error('getHabitCompletionDates requires userId')
  if (!habitId) throw new Error('getHabitCompletionDates requires habitId')

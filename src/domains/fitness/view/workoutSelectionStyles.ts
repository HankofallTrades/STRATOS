export const workoutPanelClassName =
  "stone-panel text-foreground";

export const workoutDialogClassName = `${workoutPanelClassName} rounded-[28px] p-5 sm:max-w-md`;

export const workoutPopoverClassName = `stone-surface w-[220px] rounded-[18px] p-2 text-foreground`;

export const workoutMenuInputClassName =
  "stone-inset h-11 rounded-[16px] text-sm text-foreground shadow-none placeholder:text-muted-foreground/80 focus-visible:ring-0 focus-visible:ring-offset-0";

export const workoutMenuOptionClassName =
  "h-10 w-full justify-start rounded-[12px] border border-transparent px-3 text-[13px] font-medium text-foreground/88 hover:bg-white/[0.03] hover:text-foreground disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-foreground/88";

export const workoutMenuSectionClassName =
  "stone-surface rounded-[20px] p-3";

export const workoutMenuSecondaryButtonClassName =
  "stone-chip h-11 rounded-[14px] text-foreground/88 hover:bg-white/[0.04] hover:text-foreground";

export const workoutMenuPrimaryButtonClassName =
  "h-11 rounded-[16px] bg-[#1f5146] text-[#eef7f4] hover:bg-[#275f52]";

export const workoutMenuSelectTriggerClassName =
  "stone-inset h-11 rounded-[16px] text-sm text-foreground shadow-none focus:ring-0 focus:ring-offset-0";

export const workoutMenuSelectContentClassName =
  `${workoutPopoverClassName} rounded-[18px] p-1`;

export const workoutMenuSelectItemClassName =
  "rounded-[12px] py-2.5 pl-8 pr-3 text-sm text-foreground/88 focus:bg-white/[0.04] focus:text-foreground";

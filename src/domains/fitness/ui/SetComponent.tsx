...
const SWIPE_COPY_THRESHOLD = 28;
const SWIPE_COPY_VELOCITY_THRESHOLD = 320;
...
const SetComponent: React.FC<SetComponentProps> = ({
  ...
  const wrapCellContent = (children: React.ReactNode) => {
    if (!shouldAnimateCellContent) {
      return children;
    }
    ...
    return (
      <div className="overflow-hidden">
        <motion.div
          initial={{ opacity: 0, y: contentInitialY }}
  ...
  const wrapCopyableCellContent = (
    children: React.ReactNode,
    onSwipeCopy?: () => void
  ) => {
    const content = wrapCellContent(children);
    ...
    return (
      <motion.div
        drag="y"
        dragDirectionLock
        dragConstraints={{ top: 0, bottom: 34 }}
        dragElastic={{ top: 0, bottom: 0.12 }}
        dragMomentum={false}
        dragSnapToOrigin
        data-card-swipe-block="true"
        className="touch-none"
        title="Swipe down to copy into the latest set"
        whileDrag={{ scale: 0.99 }}
        onDragEnd={(_, info) => {
          if (
            info.offset.y >= SWIPE_COPY_THRESHOLD ||
            info.velocity.y >= SWIPE_COPY_VELOCITY_THRESHOLD
          ) {
            onSwipeCopy();
          }
        }}
  ...
  if (isCardioSet(set)) {
    return (
      ...
      >
        ...
        )}>{wrapCellContent(setIndex + 1)}</TableCell>
          ...
          <>
            <div>
              <input
                ...
                className={strengthInputClassName}
      ...
      {isStatic ? (
        ...
        <TableCell className="relative w-[84px] px-2 py-2 align-middle">
          {wrapCopyableCellContent(
            ...
            isCompleted ? onSwipeCopyReps : undefined
          )}
        </TableCell>
      )}
      <TableCell className="w-[52px] align-middle px-0 py-0">

...
const getSourceExerciseId = (
    source: fitnessRepo.ExerciseSetRow | fitnessRepo.LatestSingleExerciseLogData,
    fallbackExerciseId: string
) => {
    return 'exercise_id' in source ? source.exercise_id : fallbackExerciseId;
};

export const useSingleExerciseLog = (isOpen: boolean, defaultLogData?: fitnessRepo.LatestSingleExerciseLogData | null) => {
    ...
    // Business Logic
    const updateField = useCallback(<TField extends keyof SingleLogForm>(field: TField, value: SingleLogForm[TField]) => {
    ...
    useEffect(() => {
        ...
        if (source && form.exerciseId === getSourceExerciseId(source, form.exerciseId)) {
            setForm(prev => ({
                ...prev,

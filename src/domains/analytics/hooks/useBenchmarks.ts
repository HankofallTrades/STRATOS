...
const toBenchmarkProfile = (profile: ProfileRow | null): BenchmarkProfile | null => {
    if (!profile) {
        return null;
    }
...
export const useBenchmarks = (userId: string | undefined, exercises: Exercise[]) => {
    ...
    const { data: userProfile, isLoading: isLoadingProfile, error: errorProfile } = useQuery<BenchmarkProfile | null, Error>({
        queryKey: ['userProfile', userId],
        queryFn: async () => {
            if (!userId) return null;
            return toBenchmarkProfile(await fetchUserProfile(userId));
        },
        enabled: !!userId,
        staleTime: 5 * 60 * 1000,
    });
